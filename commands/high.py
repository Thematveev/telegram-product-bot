...
def high_controller(message, bot):
    bot.send_message(
        message.chat.id,
        "Вы выбрали фильтрацию по возрастанию. Выберите одну из категорий",
...
def amount_controller(message, bot):
    ...
    sorted_products = sorted(get_products(temp_data[message.chat.id]['category']), key=lambda x: x['price'], reverse=True)[
                      :int(temp_data[message.chat.id]['amount'])]
